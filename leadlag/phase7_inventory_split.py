...
def starts(edge,margin):
    ...
    for i,x in enumerate(edge):
        ...
        inside=hit
    return out
def simulate(date,capital,margin,lat,qty,stress_bp,split,arr):
    ...
    # split = share of each venue's capital prepared for A (UP KRW + BN BTC).
    # Remaining share is prepared for B (UP BTC + BN USDT).
    fx0=(usb[0]+usa[0])/2;bnmid0=(bnb[0]+bna[0])/2;upmid0=(ubb[0]+uba[0])/2
    up_krw=float(capital*split);up_btc=float(capital*(1-split)/upmid0)
    bn_btc=float(capital*split/(bnmid0*fx0));bn_usdt=float(capital*(1-split)/fx0)
    ...
    # Forced end-of-day rebalance to the starting inventory mix.
    net_qty=(execA-execB)*qty
    imbalance=abs(net_qty)
    rebalance_pnl=0.0
    last=len(ubb)-1
    if net_qty>1e-12:
        # Close net A flow with B.
        rq=min(net_qty,up_btc,bn_usdt/(bna[last]*(1+bnf)) if bna[last]>0 else 0)
        if rq>1e-12: rebalance_pnl=float((ubb[last]*(1-upf)-bna[last]*usa[last]*(1+bnf))*rq)
    elif net_qty<-1e-12:
        # Close net B flow with A.
        rq=min(-net_qty,bn_btc,up_krw/(uba[last]*(1+upf)) if uba[last]>0 else 0)
        if rq>1e-12: rebalance_pnl=float((bnb[last]*usb[last]*(1-bnf)-uba[last]*(1+upf))*rq)
    net=gross-stress_cost+rebalance_pnl
    return {"capital_each_krw":capital,"split_A":split,"margin_bp":margin,"lat_ms":lat,"qty_btc":qty,"stress_bp":stress_bp,
...
def main():
    log("PHASE7_SPLIT_START",dates=DATES,capitals=CAPITALS,margins=MARGINS,lats=LATS,qtys=QTYS,stress=STRESS,splits=SPLITS)
    results=[];fails=[]
    for date in DATES:
        try:
                ...
                for m in MARGINS:
                    for lat in LATS:
                        for q in QTYS:
                            for sbp in STRESS:
                                for split in SPLITS:
                                    z=simulate(date,c,m,lat,q,sbp,split,arr);z["date"]=date;results.append(z)
    ...
    agg={}
    for z in results:
        key=(z["capital_each_krw"],z["split_A"],z["margin_bp"],z["lat_ms"],z["qty_btc"],z["stress_bp"])
    ...
    for key,a in sorted(agg.items()):
        c,split,m,lat,q,sbp=key
        compact.append({"capital_each_krw":c,"split_A":split,"margin_bp":m,"lat_ms":lat,"qty_btc":q,"stress_bp":sbp,
    ...
    print("PHASE7_COMPACT="+json.dumps({"failures":fails,"scenarios":compact},separators=(",",":")),flush=True)
    log("PHASE7_DONE",rows=len(results),failures=len(fails))
